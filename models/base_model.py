...
class BaseModel(ABC, Configurable):
    ...
    def __init__(self, opt):
        ...
        """
        self.opt = opt
        self.isTrain, self.isTest = opt.isTrain, opt.isTest
    ...
    def setup(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            if opt.load_epoch == 'latest':
                versions = [int(os.path.basename(x).split('_')[0]) for x in glob.glob(os.path.join(self.save_dir, '*.pth')) if 'latest' not in x]
                current_epoch = max(versions) if len(versions) > 0 else 'latest'
            else:
