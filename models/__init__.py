...
def find_network_using_name(network_name):
    networklib = importlib.import_module('models.networks')
    network = None
    ...
    if network is None:
        print("In networks.py, there should be a subclass of torch.nn.Module with class name that matches %s in lowercase." % (target_network_name))
        exit(0)
